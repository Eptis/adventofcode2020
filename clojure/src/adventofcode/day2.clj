...
; Part 1
(defn parse-pw [pw]
  (let [parsed (re-matches #"(\d+)\-(\d+)\s(\w)\:\s(\w+)" pw)
        min (Integer/parseInt (nth parsed 1))
        max (Integer/parseInt (nth parsed 2))
        char (nth parsed 3)
        pass (nth parsed 4)]
    {:min min :max max :char char :pass pass}))


(defn is-valid? [password]
  (let [{min :min max :max char :char pass :pass} password
        ch (first (char-array char))
